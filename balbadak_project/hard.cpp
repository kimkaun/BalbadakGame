...
// hardLevel 함수 선언
void hardLevel(sf::RenderWindow& window, const sf::Font& font) {
    ...
    // 시계 설정
    sf::Clock clock;
    float spawnInterval = 0.5f; // 0.5초 간격으로 이미지 변경
    ...
    while (window.isOpen()) {
        ...
        if (score >= 100) {
            ...
            if (!eggTexture.loadFromFile("C:\\work\\c++_projects\\balbadak_project\\img\\greenegg.png")) {
                return;  // 이미지 로드 실패 시 종료
            }
            ...
            congratulationText.setFillColor(sf::Color(79, 245, 145));
            congratulationText.setString(L"축하드려요! 이제 알을 눌러보세요!");
            congratulationText.setPosition(98, 70);

            // 텍스처 로드
            sf::Texture newTexture;
            if (!newTexture.loadFromFile("C:\\work\\c++_projects\\balbadak_project\\img\\greencat.png")) {
                return;
            };
            sf::Sprite newSprite;
            newSprite.setTexture(newTexture);
            // 크기 조정: 텍스처가 설정된 이후에 getLocalBounds() 사용
            scaleX = 352.0f / newSprite.getLocalBounds().width;
            scaleY = 289.0f / newSprite.getLocalBounds().height;
            newSprite.setScale(scaleX, scaleY);
            newSprite.setPosition(224, 169);       // 고양이 이미지 위치
